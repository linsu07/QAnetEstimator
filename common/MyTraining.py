...
def train_and_evaluate(estimator, train_spec, eval_spec, check_point_listener):
    ...
    executor = _TrainingExecutor(estimator=estimator, train_spec=train_spec,
                                 eval_spec=eval_spec, listener = check_point_listener)
...
class _TrainingExecutor(object):
    ...
    def __init__(self, estimator, train_spec, eval_spec,listener=None):
        if not isinstance(estimator, estimator_lib.Estimator):
            raise TypeError('`estimator` must have type `tf.estimator.Estimator`.')
    ...
    def run_chief(self):
        ...
        return self._start_distributed_training(saving_listeners=self.listener)
